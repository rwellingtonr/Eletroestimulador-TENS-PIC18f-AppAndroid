...
void receiveData()
{
  ...
  delay_ms(10);
  set = strstr(msg,"on"); //caso tenha a informa�ao de on na msg ele faz o proximo if
  if (set != 0)
   {
  ...
  set = strstr(msg,"off"); //caso tenha a informa�ao de off na msg ele faz o proximo if
  if (set != 0)
   {
    Lcd_Cmd(_LCD_CLEAR);               // Clear display
